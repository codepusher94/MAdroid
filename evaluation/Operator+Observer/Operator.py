...
from approach.utils import mes_compress_agent
from approach.utils import text_generate_agent
        ...
        if self.execute_state == 2:
            ...
            if len(matches) == 2 and matches[0].lower() == "tap":
                ...
                if pool.current_device > int(pool.device_total_num):
                    response = {"action_infos": [{"action_type": android_controller.ActionType.ACTIVATE}], "status": 0, "device_switch": True, "matches": matches}
                else:
